...
import BubbleMenuPopUp from "./bubbleMenu";

const TiptapEditor = ({ content, onChange }) => {
	return (
		<div className="rounded-lg border p-4 bg-white shadow-md max-h-[calc(100vh-12rem)] min-h-[300px] overflow-y-auto p-6 rounded-b-lg shadow-lg">
			<EditorProvider
				slotBefore={<ToolBar />}
				extensions={[StarterKit]}
				content={content}
				...
				editorProps={{
					attributes: {
						class:
							" p-6 prose prose-sm sm:prose lg:prose-lg xl:prose-2xl mx-auto focus:outline-none",
					},
				}}
				immediatelyRender={false}
			>
				<BubbleMenuPopUp />
				<EditorContent className="max-h-[calc(100vh-12rem)] min-h-[250px] overflow-y-auto" />
			</EditorProvider>
		</div>
	);
};
